...
public class IntegrationServiceImpl implements IntegrationService {
    ...
    public MarshallingResponse<ProjectDiagram> getDiagramByPath(final Path path, final MarshallingRequest.Mode mode) {
        ...
        if (path.getFileName().contains("error")) {
            ...
            for (int i = 0; i < 100; i++) {
                int key = i % 3;
                messages.add(new MarshallingMessage("uuid_" + i, i, Violation.Type.ERROR, "The error message for error: " + key, "MarshallingMessage.messageKey" + key, Arrays.asList("param1", "param2")));
            }
        ...
        if (path.getFileName().contains("okmessages")) {
            //let continue the code, this case emulate the case were the migration can be done, but changes will be produced, the user must be alerted.
            for (int i = 0; i < 100; i++) {
                int key = i % 3;
        ...
        try (InputStream is = loadPath(path)) {
            ...
            if (marshallingResponse.getState() == MarshallingResponse.State.ERROR) {
                ...
                return new MarshallingResponse.MarshallingResponseBuilder<ProjectDiagram>()
                        //.messages(marshallingResponse.getMessages())
                        //TODO WM, remove this messages and set the messages coming from the marshallingResponse again.
                        .messages(messages)
                        .state(marshallingResponse.getState())
                        .result(diagram)
                        .build();
