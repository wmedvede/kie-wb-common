...
public class BPMNProjectSessionDiagramOpenedHandler extends BPMNProjectBaseSessionDiagramHandler implements SessionDiagramOpenedHandler {
    ...
    public BPMNProjectSessionDiagramOpenedHandler() {
    ...
    public void onSessionDiagramOpened(final ClientSession clientSession, final boolean readonly) {
        final AbstractCanvasHandler canvasHandler = (AbstractCanvasHandler) clientSession.getCanvasHandler();
        final ProjectMetadata metadata = (ProjectMetadata) canvasHandler.getDiagram().getMetadata();
        if (!readonly && (metadata.getDiagramSVGPath() == null || metadata.getDiagramSVGGenerator() == ProjectMetadata.SVGGenerator.JBPM_DESIGNER)) {
            super.saveOrUpdateDiagram(clientSession);
        }
    }
}
