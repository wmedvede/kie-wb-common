...
public class CaseManagementDirectDiagramMarshaller extends BaseDirectDiagramMarshaller {
    ...
    private void preMarshallProcess(final Diagram<Graph, Metadata> diagram) {
        ...
        StreamSupport.stream(diagram.getGraph().nodes().spliterator(), false)
                .filter(node -> !(((View) ((NodeImpl) node).getContent()).getDefinition() instanceof CaseManagementDiagram))
                .forEach(node -> {
    ...
    private void postUnmarshallProcess(final Graph<DefinitionSet, Node> graph) {
        StreamSupport.stream(graph.nodes().spliterator(), false)
            ...
            final Node startNode = ((Edge) root.getOutEdges().get(0)).getTargetNode();
            if (((View) startNode.getContent()).getDefinition() instanceof StartNoneEvent) {
                deleteChild(root, startNode);
                graph.removeNode(startNode.getUUID());
            }

            final Node endNode = ((Edge) root.getOutEdges().get(root.getOutEdges().size() - 1)).getTargetNode();
            if (((View) endNode.getContent()).getDefinition() instanceof EndNoneEvent) {
                deleteChild(root, endNode);
                graph.removeNode(endNode.getUUID());
            }

            // remove sequence flow between elements in stages
            root.getOutEdges().stream().map(e -> ((Edge) e).getTargetNode())
                    .filter(n -> ((View) ((Node) n).getContent()).getDefinition() instanceof AdHocSubprocess)
