...
@ApplicationScoped
public class DataSourceDefRegistryImpl
        implements DataSourceDefRegistry {

    private Map<String, DataSourceDef> entries = new HashMap<>( );

    @Override
    public void registerDataSourceDef( DataSourceDef dataSourceDef ) {
        entries.put( dataSourceDef.getUuid(), dataSourceDef );
    }

    @Override
    public void deRegisterDataSourceDef( String uuid ) {
        entries.remove( uuid );
    }

    @Override
    public DataSourceDef getDataSourceDef( String uuid ) {
