...
public class DataSourceProviderRegistryImpl
    ...
    @Override
    public void registerProvider( DataSourceProvider dataSourceProvider ) {
        providers.add( dataSourceProvider );
    }

    @Override
    public DataSourceProvider getProvider( DataSourceDefType type ) {
        for ( DataSourceProvider provider : providers ) {
            if ( provider.accepts( type ) ) {
                return provider;
            }
        }
        return null;
