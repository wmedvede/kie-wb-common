...
public class MavenBuildOptionExtension
    ...
    private static final String LINK_NAME = "Maven build";

    private Caller<MavenBuildService> mavenBuildService;

    private Event<NotificationEvent> notificationEvent;

    private Event<BuildResults> buildResultsEvent;

    public MavenBuildOptionExtension( ) {
    }
    ...
    private ClickHandler createClickHandler( final Project project ) {
        return event -> {
            BusyPopup.showMessage( "Executing Project Maven Build" );
            mavenBuildService.call( getBuildSuccessCallback(), getErrorCallback() ).build( project, false );
        };
    }

    private RemoteCallback< BuildResults > getBuildSuccessCallback() {
        return result -> {
